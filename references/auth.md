...
```bash
curl "https://api.clkd.xyz/nonce?address=0xYourAuthAddress"
```
...
```bash
curl -X POST https://api.clkd.xyz/verify \
...
```bash
curl -X POST https://api.clkd.xyz/logout \
...
| `GET /nonce` | 20/min |
| `POST /verify` | 10/min |
| `GET /accounts/{id}/balance` | 60/min |
| ENS gateway | 15/min |
...
```bash
curl https://api.clkd.xyz/.well-known/hpke-public-key
```
